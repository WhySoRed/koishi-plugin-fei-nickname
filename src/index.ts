...
export const Config: Schema<Config> = Schema.intersect([
  Schema.object({
    defaultNickName: Schema.string()
      .default("那个谁")
      .description("获取用户名失败时的默认名称"),
    globalEnableNickName: Schema.boolean()
      .default(true)
      .description("开启“自称”功能，关闭会清空自称数据"),
    globalEnableNickNameGiven: Schema.boolean()
      .default(true)
      .description("开启“外号”功能，关闭会清空外号数据"),
    globalEnableDoSomeThing: Schema.boolean()
      .default(true)
      .description("开启“呼唤”功能，关闭也不会清空什么数据"),
    globalEnableBlacklist: Schema.boolean()
      .default(true)
      .description("开启“拉黑”子功能，关闭会清空黑名单数据"),
  }),
  Schema.object({
    enableEasyFind: Schema.boolean()
      .default(false)
      .description('为"呼唤.找"添加一个快捷指令"找"'),
    enableEasyDoSth: Schema.boolean()
      .default(false)
      .description('为"呼唤.我要"添加一个快捷指令"/"'),
    enableAt2Find: Schema.boolean()
      .default(false)
      .description('让以"@"开始却没有艾特到用户的群聊文本被视为"呼唤.找"'),
  }).description("快捷指令"),
]);
...
外号.拉黑 来禁止所有人给你起外号（并清除你的全部外号）`;
    ...
    ctx
      ...
      .action(async ({ args, session }) => {
        if (h.select(args[0], "at").length != 1)
          return "请确定是否正确艾特了对方";
        if (args[1] === undefined) return "请提供外号";
        if (args.length > 2) return "外号不能有空格";
        if (
          await nickNameDo.checkBeBlacklistGiven(
            session,
            h.select(args[0], "at")[0].attrs.id
          )
        )
          return "你被对方拉黑了，不能给ta起外号";
        if (
          await nickNameDo.addNickGiven(
            session,
            h.select(args[0], "at")[0].attrs.id,
            args[1]
          )
        )
          return `你给 ${await nickNameDo.getNick(
            session,
            h.select(args[0], "at")[0].attrs.id
          )} 起了外号 ${args[1]} `;
    ...
    ctx
      ...
      .action(async ({ args, session }) => {
        ...
        if (args[1] === undefined) return "请提供外号";
        if (
          await nickNameDo.removeNickGiven(
            session,
            h.select(args[0], "at")[0].attrs.id,
            args[1]
          )
          ...
          return "取消失败，只有起这个外号的人和被起外号的人可以取消，或者你要取消的外号不存在";
      });
    ...
    ctx
      ...
      .action(async ({ args, session }) => {
        ...
        //查看别人的外号
          ...
          if (args[1] === undefined) {
            nickNameData = await nickNameDo.showNickGiven(
              session,
              h.select(args[0], "at")[0].attrs.id
            );
            if (nickNameData.length === 0)
              return `${await nickNameDo.getNick(
                session,
                h.select(args[0], "at")[0].attrs.id
              )} 在这个群还没有外号呀`;
          ...
          nickNameCount = await nickNameDo.countNickGiven(
            session,
            h.select(args[0], "at")[0].attrs.id
          );
        } else return "指令不太对劲呀";
        ...
        for (let i = 0; i < nickNameData.length; i++) {
          nickNameString += `${
            nickNameData[i].nickGiven
          } 是 ${await nickNameDo.getNick(
            session,
            nickNameData[i].giverUid.replace(/.*:/, "")
          )} 起的\n`;
        }
        return (
          nickNameString +
          `共有${nickNameCount}个外号${
            nickNameCount > 10 ? "，剩余外号可以输入对应的页数来查看" : ""
          }`
        );
      });
      ...
      ctx
        ...
        .action(async ({ args, session }) => {
          if (args[0] === undefined) {
            ...
            else return `现在可以被起外号了~`;
          } else if (
            session.event.user.id === h.select(args[0], "at")[0].attrs.id
          ) {
            return "你在干嘛...";
          } else if (h.select(args[0], "at").length === 1) {
            if (
              await nickNameDo.switchBlacklistGiven(
                session,
                h.select(args[0], "at")[0].attrs.id
              )
            )
              return `已禁止${await nickNameDo.getNick(
                session,
                h.select(args[0], "at")[0].attrs.id
              )}给你起外号，ta给你起的外号也清空啦，要再发送本指令才会允许~`;
            else
              return `已取消禁止，现在可以被${await nickNameDo.getNick(
                session,
                h.select(args[0], "at")[0].attrs.id
              )}起外号了`;
          }
      ...
      .action(async ({ args, session }, message) => {
        ...
        if (findId.length === 1) {
          ...
          if (await nickNameDo.checkBeBlacklistDosth(session, findId[0]))
            return (
              (await nickNameDo.getNick(session, findId[0])) +
              "把你拉黑了，不能找ta~"
            );
          const senderNick = await nickNameDo.getNickGiven(
            session,
          ...
          return (
            ...
            h.at(findId[0]) +
            " " +
            (message.length < 4 ? `${message}${message}` : message) +
            "，" +
            (senderNick === config.defaultNickName ? "有人" : senderNick) +
            "找你呀"
          );
        }
        if (findId.length > 1) {
          if (args[1] === undefined) {
            ...
            for (let i = 0; i < findId.length; i++) {
              findString += `${i + 1}. ${await nickNameDo.getNick(
                session,
                findId[i]
              )} 账号：${findId[i]}\n`;
            }
            findString += "里面的谁呀？在后面带上序号再发送一次指令就可以了";
          } else if (Number.isNaN(+args[1]) || +args[1] > findId.length)
            return "序号有误";
          else {
            if (findId[+args[1] - 1] === session.selfId) return "你在找我吗？";
            if (findId[+args[1] - 1] === session.event.user.id)
              return "你在找自己吗？";
            if (
              await nickNameDo.checkBeBlacklistDosth(
                session,
                findId[+args[1] - 1]
              )
            )
              return (
                (await nickNameDo.getNick(session, findId[+args[1] - 1])) +
                "把你拉黑了，不能找ta~"
              );
            const senderNick = await nickNameDo.getNickGiven(
              session,
      ...
      .action(async ({ args, session }) => {
        ...
        if (h.select(args[1], "at").length === 1) {
          if (
            await nickNameDo.checkBeBlacklistDosth(
              session,
              h.select(args[1], "at")[0].attrs.id
            )
          )
            return (
              (await nickNameDo.getNick(
                session,
                h.select(args[1], "at")[0].attrs.id
              )) + "把你拉黑了，不能对ta动手动脚~"
            );
          return (
            (await nickNameDo.getNick(session)) +
            args[0] +
            "了" +
            (await nickNameDo.getNickGiven(
              session,
              h.select(args[1], "at")[0].attrs.id
            )) +
            args.slice(2).join(" ")
        ...
        if (h.select(args[1], "at").length > 1) return "只能一个人哦~";
          ...
          if (findId.length === 0) return "没有找到这个人呀";
          if (findId.length === 1) {
            if (findId[0] === session.event.user.id)
              return (
                (await nickNameDo.getNick(session)) +
                args[0] +
                "了自己" +
          ...
          if (findId.length > 1) {
            if (args[2] === undefined) {
              ...
              findString += "里面的谁呀？在后面带上序号再发送一次指令就可以了";
            } else if (Number.isNaN(+args[2]) || +args[2] > findId.length)
              return "序号有误";
            else if (
              await nickNameDo.checkBeBlacklistDosth(
                session,
      ...
      ctx
        ...
        .action(async ({ args, session }) => {
          if (args[0] === undefined) {
            ...
            if (
              await nickNameDo.switchBlacklistDosth(
                session,
                h.select(args[0], "at")[0].attrs.id
              )
            )
              return `已禁止${args[0]}对你动手动脚，要再发送本指令才会允许~`;
            else return `已取消禁止，现在可以被${args[0]}做什么了`;
          }
          return "指令不太对劲呀";
        });
    }
