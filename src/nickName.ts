...
export const nickNameDo = {
  init: async (ctx: Context, config: Config): Promise<void> =>
    await nickNameDo._ininitialize(ctx, config),
  getNick: async (
    session: Session,
    id?: string | string[]
  ): Promise<string | string[]> =>
    await nickNameDo._nick.get(
      session,
      id ? await nickNameDo._id2uid(session, id) : undefined
    ),
  find: async (session: Session, nickGiven: string): Promise<string[]> =>
    await nickNameDo._uid2id(await nickNameDo._find(session, nickGiven)),
  addNick: async (session: Session, nickName: string): Promise<boolean> =>
    await nickNameDo._nick.add(session, nickName),
  removeNick: async (
    session: Session /*, nickName: string*/
  ): Promise<boolean> => await nickNameDo._nick.remove(session /*, nickName*/),
  getNickGiven: async (
    session: Session,
    ownerId?: string | string[],
    giverId?: string
  ): Promise<string | string[]> =>
    await nickNameDo._nickGiven.get(
      session,
      ownerId ? await nickNameDo._id2uid(session, ownerId) : undefined,
      giverId ? await nickNameDo._id2uid(session, giverId) : undefined
    ),
  addNickGiven: async (
    session: Session,
    id: string,
    ...
    await nickNameDo._nickGiven.add(
      session,
      await nickNameDo._id2uid(session, id),
      nickGiven
    ),
  showNickGiven: async (session: Session, id: string, page?: number) =>
    await nickNameDo._nickGiven.show(
      session,
      await nickNameDo._id2uid(session, id),
      page
    ),
  showOwnNickGiven: async (session: Session, page?: number) =>
    await nickNameDo._nickGiven.show(session, undefined, page),
  countNickGiven: async (session: Session, id?: string): Promise<number> =>
    await nickNameDo._nickGiven.count(
      session,
      id ? await nickNameDo._id2uid(session, id) : undefined
    ),
  removeNickGiven: async (
    session: Session,
    id: string,
    nickGiven: string
  ): Promise<boolean> =>
    await nickNameDo._nickGiven.remove(
      session,
      await nickNameDo._id2uid(session, id),
      nickGiven
    ),
  allBlacklistGiven: async (session: Session): Promise<boolean> =>
    await nickNameDo._blacklist.all(session, "given"),
  allBlacklistDosth: async (session: Session): Promise<boolean> =>
    await nickNameDo._blacklist.all(session, "dosth"),
  switchBlacklistGiven: async (
    session: Session,
    id: string
  ): Promise<boolean> =>
    await nickNameDo._blacklist.switch(
      session,
      await nickNameDo._id2uid(session, id),
      "given"
    ),
  switchBlacklistDosth: async (
    session: Session,
    id: string
  ): Promise<boolean> =>
    await nickNameDo._blacklist.switch(
  ...
  checkBeBlacklistGiven: async (
    session: Session,
    id: string
  ): Promise<boolean> =>
    await nickNameDo._blacklist.check(
      session,
      await nickNameDo._id2uid(session, id),
      "given"
    ),
  checkBeBlacklistDosth: async (
    session: Session,
    id: string
  ): Promise<boolean> =>
    await nickNameDo._blacklist.check(
  ...
  _id2uid: async (session: Session, id: string | string[]) => {
    if (Array.isArray(id))
      return Promise.all(
        id.map(async (id) => (await nickNameDo._id2uid(session, id)) as string)
      );
  ...
  _uid2id: async (uid: string | string[]) => {
    if (Array.isArray(uid))
      return await Promise.all(
        uid.map(async (uid) => (await nickNameDo._uid2id(uid)) as string)
      );
    return uid.replace(/.*:/, "");
  },
  ...
  _nick: {
    ...
    get: async (session: Session, uid?: string | string[]) => {
      ...
      if (Array.isArray(uid))
        return Promise.all(
          uid.map(
            async (uid) => (await nickNameDo._nick.get(session, uid)) as string
          )
        );
      const ctx = session.app;
      let nickName = (
        await ctx.database.get("nnNickData", { ownerUid: uid })
      )[0]?.nickName;
          ...
          if (session.event.channel.type) {
            const name = await Promise.any(
              (
                ...
                async (guild) =>
                  (
                    await session.bot.getGuildMember(
                      guild.id,
                      uid.replace(/.*:/, "")
                    )
                  ).user.name
    ...
    add: async (session: Session, nickName: string) => {
      const ctx = session.app;
      await ctx.database.upsert("nnNickData", [
        { ownerUid: session.uid, nickName: nickName },
      ]);
      return true;
    },
    remove: async (session: Session /*, nickName: string*/) => {
      const ctx = session.app;
      await ctx.database.remove("nnNickData", {
        ownerUid: session.uid /*, nickName: nickName*/,
      });
      return true;
    },
  ...
  _nickGiven: {
    ...
    get: async (
      ...
      if (Array.isArray(ownerUid))
        return Promise.all(
          ownerUid.map(
            async (ownerUid) =>
              (await nickNameDo._nickGiven.get(session, ownerUid)) as string
          )
        );
      const ctx = session.app;
      let nickGivenList = await ctx.database.get("nnGivenData", {
        cid: session.cid,
        ownerUid: ownerUid,
        giverUid: giverUid,
      });
      if (nickGivenList.length) {
        return nickGivenList[Math.floor(Math.random() * nickGivenList.length)][
          "nickGiven"
        ];
      } else {
        nickGivenList = await ctx.database.get("nnGivenData", {
          cid: session.cid,
          ownerUid: ownerUid,
        });
        if (nickGivenList.length) {
          return nickGivenList[
            Math.floor(Math.random() * nickGivenList.length)
          ]["nickGiven"];
        }
      }
    ...
    add: async (session: Session, uid: string, nickGiven: string) => {
      const ctx = session.app;
      if (await nickNameDo._blacklist.check(session, uid, "given"))
        return false;
      try {
        await ctx.database.create("nnGivenData", {
          cid: session.cid,
          ownerUid: uid,
          giverUid: session.uid,
          nickGiven: nickGiven,
        });
      } catch (err) {
        return false;
    ...
    remove: async (session: Session, uid: string, nickGiven: string) => {
      const ctx = session.app;
      const data = (
        await ctx.database.get("nnGivenData", {
          cid: session.cid,
          ownerUid: uid,
          nickGiven: nickGiven,
        })
      )[0];
      if (data.giverUid != session.uid && data.ownerUid != session.uid)
        return false;
      await ctx.database.remove("nnGivenData", {
        cid: session.cid,
        ownerUid: uid,
        nickGiven: nickGiven,
      });
    ...
    show: async (session: Session, uid?: string, page?: number) => {
      ...
      return ctx.database.get(
        "nnGivenData",
        { cid: session.cid, ownerUid: uid },
        { limit: 10, offset: page ? 10 * (page - 1) : 0 }
      );
    },
  ...
  _blacklist: {
    add: async (session: Session, uid: string, type: "given" | "dosth") => {
      const ctx = session.app;
      await ctx.database.create("nnBlacklistData", {
        fromUid: session.uid,
        toUid: uid,
        type: type,
      });
    ...
    remove: async (session: Session, uid: string, type: "given" | "dosth") => {
      const ctx = session.app;
      await ctx.database.remove("nnBlacklistData", {
        fromUid: session.uid,
        toUid: uid,
        type: type,
      });
    ...
    switch: async (session: Session, uid: string, type: "given" | "dosth") => {
      const ctx = session.app;
      const blacklist = await ctx.database.get("nnBlacklistData", {
        fromUid: session.uid,
        toUid: uid,
        type: type,
      });
    ...
    check: async (
      ...
      if (
        (
          await ctx.database.get("nnBlacklistData", {
            fromUid: uid,
            toUid: uid,
            type: type,
          })
  ...
  _find: async (session: Session, nickGiven: string): Promise<string[]> => {
    ...
    if (ownerUid[0] !== undefined) return ownerUid;
      ...
      const ownerUidAll = await ctx.database.get("nnNickData", {
        nickName: nickGiven,
      });
      //对所有具有该称号的用户进行筛选
      ownerUid = await Promise.all(
        ownerUidAll
          .map(async (data) => {
    ...
    //对群昵称进行筛选，失败则对账户名进行筛选
      ...
      const guildMemverList = (
        await session.bot.getGuildMemberList(session.event.guild.id)
      ).data;
      ownerUid = guildMemverList
        .filter((member) => member.nick == nickGiven)
        .map((member) => session.platform + ":" + member.user.id);
      if (ownerUid[0] !== undefined) return ownerUid;
      else
        ownerUid = guildMemverList
          .filter((member) => member.user.name == nickGiven)
          .map((member) => session.platform + ":" + member.user.id);
    }
    return ownerUid;
  },
